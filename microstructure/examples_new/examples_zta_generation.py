...
def generate_zta_rves(rve_dim=32, n_obj=50, n_vol_fracs=3, n_rves=3, 
    ...
    for i in range(1,n_vol_fracs):
        ...
        for j in range(0,n_rves):
            ...
            geo_comm.introduce_objects(N=round(frac*n_obj), object_type='sphere', 
                                        shape_description={'radius': 5.0e-6})
            geo_comm.introduce_objects(N=round((1-frac)*n_obj), object_type='sphere', 
...
outpath = generate_zta_rves(rve_dim=32, n_obj=20, n_vol_fracs=6, n_rves=2, outpath='output')
