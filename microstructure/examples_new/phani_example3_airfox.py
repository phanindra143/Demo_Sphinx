...
frac = Input_frac/100 
Per_Al2O3 = 100 - (Input_ZrO2 + Input_Porosity)
Per_ZrO2 = Input_ZrO2

# set target properties of the RVE
n_rves = [1,2,3,4,5]                # number of RVEs to generate            
...
for dim in range(len(n_rves)):    
    ...
    base_path = Path('Airfox_list_nrves_nobj_50')/f'n_rves_{n_rves[dim]}'
    for j in range(n_rves[dim]):
     # launch GeoVal
